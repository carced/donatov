(function () {
  ...
  function initDependentSelects() {
    document.querySelectorAll('select.crypto-field-input[data-depends-on]').forEach((childSelect) => {
      const parentKey = childSelect.dataset.dependsOn;
      const parentSelect = document.querySelector(
        '.crypto-field-input[data-field-key="' + parentKey + '"]',
      );
      if (!parentSelect) {
        return;
      }
      ...
      function repopulate() {
        let map = {};
        try {
          map = JSON.parse(childSelect.dataset.optionsByParent || '{}');
        } catch (e) {
          map = {};
        }
        const placeholder = childSelect.querySelector('option[value=""]')?.textContent || '';
        ...
        const options = Array.isArray(map[parentValue]) ? map[parentValue] : [];
