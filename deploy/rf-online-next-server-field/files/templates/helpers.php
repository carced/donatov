...
function field_validation_decoded(array $field): array
{
...
function field_depends_on(array $field): ?string
{
    $dependsOn = field_validation_decoded($field)['dependsOn'] ?? null;

    return is_string($dependsOn) && $dependsOn !== '' ? $dependsOn : null;
}

function field_options_by_parent_json(array $field): string
{
    $map = field_validation_decoded($field)['optionsByParent'] ?? [];

    return json_encode(is_array($map) ? $map : [], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
}

/** @return list<array{id: string, name: string}> */
function field_options(array $field): array
{
    $decoded = field_validation_decoded($field);
    if (isset($decoded['optionsByParent'])) {
        return [];
    }
    $options = [];
...
function field_placeholder(array $field): string
{
    ...
    $map = [
        ...
        'выберите сервер' => 'placeholder_select_server',
    ];
    if (isset($map[$lower])) {
        return t($map[$lower]);
    }
